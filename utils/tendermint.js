const WebSocket = WebSocket || require('websocket')
...
class Client {
	...
	async connectWs(attempt = 0) {
		if (attempt > 0) {
			console.log(`\x1b[36mretry connection (attempt ${attempt}/5) for network ${this.networkId}\x1b[0m`);
		...
		this.ws.onerror = error => {
			console.error(`\x1b[36mwebsocket connection lost for network ${this.networkId}\x1b[0m`, error.message);
		...
		this.ws.onerror = () => {
			...
			if (this.closed || attempt >= 5) {
				...
				console.log(`\x1b[31mwebsocket disconnected for network ${this.networkId}\x1b[0m`);
				return;
			}
